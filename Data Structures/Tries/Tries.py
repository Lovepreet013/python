...
class Trie:
    def __init__(self):
        self.root = Trie_Node() #Root Node
    
    def get_index(self, t):
        return ord(t) - ord("a")
        
    def insert(self, key): #Time Complexity is O(n) since we need to make n iterations
        if key is None:
        ...
        for letter in key:
            ...
            if current.children[index] is None:
                current.children[index] = Trie_Node(letter)
    ...
    def search(self, key): #Worst Time complexity is O(h), where h is length of the word
        if key is None:
            return None
    ...
    def delete(self, key): #Time Compelxity : In worst case it will be O(h) where h is length of the word.
...
# res = ["Not present in trie", "Present in trie", "by"]
# # Construct Trie
# for words in keys:
#     t.insert(words)

# for words in res:
#     print(t.search(words))
